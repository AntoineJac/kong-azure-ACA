resource "azurerm_container_app" "konnect-external-dp" {
  name     = "konnect-external-dp-${var.environment_name}"
  ...
  #   password_secret_name  = xxxxx
  #   username = xxxxx
  # }

  secret {
    name = "tls-crt"
    value = var.konnect_external_tls_crt
  }

  secret {
    name = "tls-key"
    value = var.konnect_external_tls_key
  }
  ...
  template {
    ...
    container {
      ...
      env {
        ...
        value = "${var.konnect_external_cp_endpoint}:443"
      }
      env {
        name = "KONG_CLUSTER_SERVER_NAME"
        value = var.konnect_external_cp_endpoint
      }
      env {
        name = "KONG_CLUSTER_TELEMETRY_ENDPOINT"
        value = "${var.konnect_external_telemetry_endpoint}:443"
      }
      env {
        name = "KONG_CLUSTER_TELEMETRY_SERVER_NAME"
        value = var.konnect_external_telemetry_endpoint
      }
      env {
